...
function AITransformFactory(options = {}) {
  ...
  import_logger.default.debug(`Creating AI Transformer with model: ${this.modelName}`);
  import_logger.default.debug(`Using keys - Source: "${this.promptKey}", Target: "${this.answerKey}", Context: "${this.contextKey}"`);
  this.genAIClient = new import_genai.GoogleGenAI({ apiKey: this.apiKey });
  this.chat = null;
}
async function initChat() {
  if (this.chat) return;
  import_logger.default.debug(`Initializing Gemini chat session with model: ${this.modelName}...`);
  ...
  import_logger.default.debug("Gemini chat session initialized.");
}
async function seedWithExamples(examples) {
  await this.init();
  if (!examples || !Array.isArray(examples) || examples.length === 0) {
    if (this.examplesFile) {
      import_logger.default.debug(`No examples provided, loading from file: ${this.examplesFile}`);
      examples = await import_ak_tools.default.load(import_path.default.resolve(this.examplesFile), true);
    } else {
      import_logger.default.debug("No examples provided and no examples file specified. Skipping seeding.");
      return;
    }
  }
  import_logger.default.debug(`Seeding chat with ${examples.length} transformation examples...`);
  const historyToAdd = [];
  ...
  import_logger.default.debug("Transformation examples seeded successfully.");
}
async function transformJSON(sourcePayload) {
  ...
  try {
    ...
    import_logger.default.error("Error with Gemini API:", error);
  ...
  try {
    ...
    import_logger.default.error("Error parsing Gemini response:", parseError);
...
async function transformWithValidation(sourcePayload, validatorFn, options = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      import_logger.default.debug(`Transformation and validation succeeded on attempt ${attempt + 1}`);
      ...
      if (attempt < maxRetries) {
        ...
        import_logger.default.warn(`Attempt ${attempt + 1} failed, retrying in ${delay}ms...`, error.message);
        await new Promise((res) => setTimeout(res, delay));
      } else {
        import_logger.default.error(`All ${maxRetries + 1} attempts failed`);
...
async function resetChat() {
  if (this.chat) {
    import_logger.default.debug("Resetting Gemini chat session...");
    this.chat = await this.genAIClient.chats.create({
      model: this.modelName,
      // @ts-ignore
      config: this.chatConfig,
    ...
    import_logger.default.debug("Chat session reset.");
  } else {
    import_logger.default.warn("Cannot reset chat session: chat not yet initialized.");
  }
}
function getChatHistory() {
  if (!this.chat) {
    import_logger.default.warn("Chat session not initialized. No history available.");
    return [];
  }
  return this.chat.getHistory();
}
if (import_meta.url === new URL(`file://${process.argv[1]}`).href) {
  import_logger.default.info("RUNNING AI Transformer as standalone script...");
  (async () => {
    try {
      import_logger.default.info("Initializing AI Transformer...");
      ...
      import_logger.default.info("AI Transformer initialized and seeded with examples.");
      const normalResponse = await transformer.message({ "name": "AK" });
      import_logger.default.info("Normal Payload Transformed", normalResponse);
      ...
      import_logger.default.info("Validated Payload Transformed", validatedResponse);
      if (NODE_ENV === "dev") debugger;
    } catch (error) {
      import_logger.default.error("Error in AI Transformer script:", error);
      if (NODE_ENV === "dev") debugger;
    }
  })();
}
